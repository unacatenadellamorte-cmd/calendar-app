...
export function BottomSheet({ open, title, onClose, children }: BottomSheetProps) {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (Array.from(document.querySelectorAll('[role=dialog]')).at(-1) !== panelRef.current) return;
      if (e.key === 'Escape') { e.preventDefault(); dismiss(); }
      if (e.key === 'Tab') {
        const items = panelRef.current?.querySelectorAll<HTMLElement>('button:not(:disabled), input:not(:disabled), select:not(:disabled), textarea:not(:disabled), a[href]');
        const first = items?.[0];
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex flex-col justify-end overscroll-none">
      <button type="button" aria-label="閉じる" onClick={dismiss}
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" aria-label={title} tabIndex={-1}
        style={{ transform: `translateY(${closing ? '100%' : `${offset}px`})`, transition: closing || offset === 0 ? 'transform 180ms ease-out' : 'none' }}
        className="relative mx-auto flex max-h-[90dvh] w-full max-w-2xl flex-col rounded-t-lg bg-surface-base px-4 pb-4 shadow-lg">
        <button type="button" aria-label="下にスライドして閉じる" onClick={() => { if (!suppressClick.current) dismiss(); suppressClick.current = false; }}
          className="flex min-h-11 w-full shrink-0 touch-none flex-col items-center justify-center gap-1"
          onPointerDown={(e) => { suppressClick.current = false; startY.current = e.clientY; e.currentTarget.setPointerCapture?.(e.pointerId); }}
          onPointerMove={(e) => { if (startY.current !== null) setOffset(Math.max(0, e.clientY - startY.current)); }}
          onPointerUp={(e) => {
          ...
          <span className="text-[10px] text-ink-secondary">下にスライドして閉じる</span>
        </button>

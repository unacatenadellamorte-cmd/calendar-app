...
vi.mock('@/platform/layerBack', () => ({ registerLayerBack: (close: () => void) => { back.close = close; return back.unregister; } }));
afterEach(() => { vi.useRealTimers(); vi.unstubAllGlobals(); });

describe('入力レイヤー', () => {
  it('Androidの戻るは画面遷移せず現在の閉じる処理を呼ぶ', () => {
    const close = vi.fn();
    const { rerender, unmount } = render(<BottomSheet open title="入力" onClose={vi.fn()}>本文</BottomSheet>);
    rerender(<BottomSheet open title="入力" onClose={close}>本文</BottomSheet>);
    act(() => back.close?.());
  ...
  it('つまみを下へ64px以上引くと閉じ、短い移動やキャンセルでは閉じない', () => {
    ...
    const close = vi.fn();
    render(<BottomSheet open title="入力" onClose={close}>本文</BottomSheet>);

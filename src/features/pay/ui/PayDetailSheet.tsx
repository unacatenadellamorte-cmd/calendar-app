...
export function PayDetailSheet({ open, monthLabel, amount, shifts, onClose }: PayDetailSheetProps) {
  return (
    <BottomSheet open={open} title={`${monthLabel}の給料見込み`} onClose={onClose}>
      {shifts.length === 0 ? (
        <p className="text-body text-ink-secondary">{monthLabel}のシフトはまだありません。</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {shifts.map((s) => {
            const worked = s.startsAt
              ? Math.max(0, workedMinutes(s.startsAt, s.endsAt ?? s.startsAt, s.breakMinutes ?? 0))
              : 0;
              ...
              >
                <span className="text-ink-secondary">
                  ...
                  <span className="tabular">
                    {s.startsAt ? formatClock(s.startsAt) : ''}–{s.endsAt ? formatClock(s.endsAt) : ''}
                  </span>
                </span>
                <span className="tabular text-ink-primary">
                  {(worked / 60).toFixed(1)}h × ¥{wage.toLocaleString('ja-JP')} = {formatYen(subtotal)}
                </span>
              </li>
            );
          })}

...
- [x] GitHub認証後、macOS CIの成功を確認する。
- [x] Claude App・資格情報を設定し起動を確認する。
...
- このローカル検証時点ではSwift/Xcode/実機/Claudeの実起動は未実施。後続結果は下記。

### 2026-09-17 GitHub反映・クラウド検証

- PR #1をmainへマージ済み。
- CI `35168613446` とPR CI `35168904957`: Web 820テスト、Swift 6テスト、Ruby 2テスト・22アサーション、署名なしiOSビルドとWidget拡張の埋込み確認が成功。
- Claude Appはユーザーの承認に基づきcalendar-appだけにインストール。ユーザーがOAuthトークンをGitHub Secretへ登録。
- 手動の接続確認 `35169381880`: macOS上でClaudeの実起動が成功。コメント投稿経由の起動は未検証。
- main CI `35169381664`: iOSビルド成功。Webの820テストは通ったが、OnlineProviderの破棄後の非同期更新で未処理エラーが発生。処理世代を比較して破棄後の更新・送信開始を止め、回帰テストを追加。
- Swift JSONリテラルの区切りと、シークレット解除テストの非同期完了待ちもCIで発見・修正済み。
- 署名・実機での3サイズ表示、共有ストレージ反映、タップは未検証。実機受入れまでStoryは進行中とする。
